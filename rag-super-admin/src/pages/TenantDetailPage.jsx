    ...
    <div className="page-enter">
      ...
      <div style={{ display:'flex', alignItems:'flex-start', gap:16, marginBottom:24 }}>
        ...
        <div style={{ display:'flex', gap:8, flexShrink:0 }}>
          ...
          <button className="btn btn-secondary btn-sm" onClick={doImpersonate}>⊙ Impersonate</button>
        </div>
      </div>
                        ...
                        <td onClick={e => e.stopPropagation()}>
                          <div style={{ display:'flex', gap:6 }}>
                            <select
                              ...
                              onChange={e => doPromoteMember(m.user_id, e.target.value)}
                            >
...
function TenantAuditTab({ tenantId }) {
  ...
  const [loading, setLoading] = useState(true)
  const { addToast } = useToast()
  const { getActivity } = require ? null : null  // dynamic import workaround
